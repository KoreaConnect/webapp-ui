...
import { ArrowRight, Box, Calendar, Clock, Filter, Locate, MapPin, Plane, Plus, Search, Weight } from 'lucide-react';
    ...
    const filteredDeliveries = useMemo(() => {
        return MOCK_DELIVERIES.filter((item) => {
            ...
            const isKorea = (place: string) =>
                place.includes('Seoul') || place.includes('ICN') || place.includes('Pusan') || place.includes('PUS');

            if (routeFilter === 'kr-vn') {
                return isKorea(item.from) && isVietnam(item.to);
            }
            if (routeFilter === 'vn-kr') {
                return isVietnam(item.from) && isKorea(item.to);
            }
            return true;
        });
    }, [routeFilter]);
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 shadow-sm border border-border">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div className="relative group">
                        ...
                        />
                        <KakaoAddressSearch
                            onComplete={(data) => setFrom(data.address)}
                            trigger={
                                <button
                    ...
                    <div className="relative group">
                        ...
                        />
                        <KakaoAddressSearch
                            onComplete={(data) => setTo(data.address)}
                            trigger={
                                <button
                                    title="Search Korea Address"
                                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-700 rounded-lg text-zinc-400 hover:text-primary transition-all"
                ...
                <div className="grid gap-4">
                    {filteredDeliveries.length > 0 ? (
                        ...
                        <div className="text-center py-20 bg-zinc-50 dark:bg-zinc-900/50 rounded-3xl border border-dashed border-border">
                            ...
                            <Button variant="link" onClick={() => setRouteFilter('all')} className="mt-2">
                                Clear all filters
                            </Button>
                        </div>
                    )}
